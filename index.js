...
reverseCharacters("myVariableName");*/
...
function reverseCharacters(input){
  ...
  if(typeof input === "string"){
      ...
      let convertStr = input.toString();
      let reverseNum = convertStr.split('').reverse().join('');
      return Number(reverseNum); 
  }
}
reverseCharacters(123456789);
//reverseCharacters("Dung Huynh");
...
function reverseArray(arr){
  ...
  for(let i = 0; i < arr.length; i++){
    let reverseItem = reverseCharacters(arr[i]);
    retVal.unshift(reverseItem);
  }
  return retVal;
}
